...
Using the Exact Binomial Test, we can see if this observed count of success, which follows a Binomial Distribution, is unusual in a world where the dogs are equally likely to succeed or fail, i.e., perform at chance level.

* $H_0$: $p = 0.5$

* $H_a$: $p \neq 0.5$

The Exact Binomial Test indicates that the probability of observing 9 successes out of 10 under the null hypothesis is $p=0.0215 < 0.05$. There is statistically discernible support for the alternative hypothesis, that is, the dogs performed significantly above chance with the new toys in the Overhearing Condition.
...
I performed the randomization for 10,000 times. The probability of observing our observed mean ($M=-0.00833$) if the null hypothesis is true is $p=0.9847 > 0.05$. We do not have statisically discernible support for the alternative hypothesis, suggesting that there likely is not a difference between two conditions in the proportion of correct retrieval of the new toys.
...
# two-sided p-value
# We take absolute values because we're looking at values that are equal to or larger than our observed mean in both directions. # Proportion = Mean(logical vector) because T is treated as 1 and F as 0.
...
One condition of the use of Binomial Distribution is that samples have to be independent from each other. If researchers pool two observations from each dog, across 10 dogs, together, then the observations are not independent.

In addition, since the dogs went through repeated trials, the probability of success may be impacted by fatigue or learning effects, differing from 0.5.

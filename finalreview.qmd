...
alpha <- 0.05
t.critical.lower <- qt(0.025, df=n-1)
t.critical.upper <- qt(0.975, df=n-1)

ggdat <- tibble(t = seq(-5, 5, length.out=1000)) |>
...
ggplot(ggdat) +
    ...
    geom_ribbon(data=ggdat |> 
        filter(t >= t.critical.upper & t <= t.critical.lower),

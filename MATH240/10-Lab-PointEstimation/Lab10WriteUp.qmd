...
lldaltbinom <- function(prob, theta, x, size, mle = F) {
  ...
  # if mle is true (we're trying to find mle), return the negative log-likelihood. 
  # if it's false, return the log-likelihood)
  ifelse(mle, -ll, ll)
}
...
ggplot(pdc.score) +
  geom_col(aes (x= Score,
                y = Proportion)) +
  geom_hline(yintercept = 0) +
  scale_y_continuous(breaks = seq(0, 0.3, 0.1), limits = c(0, 0.4)) +
  theme_bw()+
  labs (x = "Score",
        y = "Proportion")
```
